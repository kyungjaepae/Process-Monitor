...
function generate_report {
	declare -i c=$(ls /homes/kpae/cs252s/lab2-src/reports_dir | wc -l)
	count=$(ls /homes/kpae/cs252s/lab2-src/reports_dir | wc -l)
	
	#echo count $count

	STRING="$(ls /homes/kpae/cs252s/lab2-src/reports_dir -1tr | head -1)"
	#echo $STRING
	#if [ -e /homes/kpae/cs252s/lab2-src/reports_dir/* ]; then echo "huzzah"; fi;

	#$count -gt $MAXIMUM_REPORTS
	...
	if [ $count -gt $MAXIMUM_REPORTS ]; then
		rm /homes/kpae/cs252s/lab2-src/reports_dir/$STRING
	fi
	...
	echo "PROCESS ID: $PID" > /homes/kpae/cs252s/lab2-src/reports_dir/$file_name
	echo "PROCESS NAME: $process_name" >> /homes/kpae/cs252s/lab2-src/reports_dir/$file_name
	echo "CPU USAGE: $1 %" >> /homes/kpae/cs252s/lab2-src/reports_dir/$file_name
	echo "MEMORY USAGE: $2 kB" >> /homes/kpae/cs252s/lab2-src/reports_dir/$file_name
}
...
function notify
{
	...
	if [[ $cpu_usage_int -gt $CPU_THRESHOLD || $2 -gt $MEM_THRESHOLD ]]; then
		#Check if process exceeded CPU thresholds.
		if [ $cpu_usage_int -gt $CPU_THRESHOLD ]; then
			process_name=$(cat /proc/$PID/stat | awk '{print $2}')
			echo "CPU EXCEEDED!"
			echo "Process Number: $PID" > tmp-message
			echo "Command Line: ./procnotify.sh $3 $4 $5 $6 $7 $8 $9" >> tmp-message
			echo "CPU Usage: $1 %" >> tmp-message
			echo "Memory Usage: $2 kB" >> tmp-message
			echo "Max CPU Usage: $CPU_THRESHOLD kB" >> tmp-message
			/usr/bin/mailx -s "$USER, CPU Exceeded!" $USER < tmp-message
			echo "Success"
			exit
		#case MEM exceeded	
		elif [ $2 -gt $MEM_THRESHOLD ]; then
			process_name=$(cat /proc/$PID/stat | awk '{print $2}')
			echo "MEMEORY EXCEEDED"
			echo "Process Number: $PID" > tmp-message
			echo "Command Line: ./procnotify.sh $3 $4 $5 $6 $7 $8 $9" >> tmp-message
			echo "CPU Usage: $1 %" >> tmp-message
			echo "Memory Usage: $2 kB" >> tmp-message
			echo "Max CPU Usage: $CPU_THRESHOLD kB" >> tmp-message
			/usr/bin/mailx -s "$USER, Memory Exceeded!" $USER < tmp-message
			exit
		fi
	fi
...
# $#=number of parameters, $@=all of the parameters/command line(doesn't include ./filename)
check_arguments $# $@

init $1 $@

#used for command_line
parameter_command_line=$@
...
do
	...
	notify $cpu_usage $mem_usage $parameter_command_line
